...
#         owned_shares = db.execute("SELECT symbol, sum(shares) FROM history WHERE id = %s AND symbol = %s GROUP BY symbol", (session["user_id"], symbol))[0]["sum(shares)"]
        owned_shares = db.execute("SELECT symbol, sum(shares) FROM history WHERE id, symbol = (%s, %s) GROUP BY symbol", (session["user_id"], symbol))[0]["sum(shares)"]
